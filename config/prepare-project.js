...
const runPrepareScript = () => {
  try {
    ...
    execSync(
      'pnpm install npm-check-updates prettier eslint @trivago/prettier-plugin-sort-imports -g --ignore-scripts --silent'
    );
    process.stdout.clearLine(0);
    process.stdout.cursorTo(0);
    ...
    setTimeout(() => {
      const successMessage = gradient(['#7aecdd', '#ffffff', '#f78df7'])(
